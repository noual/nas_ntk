...
from ntk.NTK import NTK, compute_min_eigenvalue
...
from search_spaces.Node import Node


class MCTSAgent:

    def __init__(self, root_node: Node, api=None, df=None, params_path=None):
        self.root = root_node
        self.api = api
        self.df = df
    ...
    def read_params(self, path):
        ...
        self.disable_tqdm = dic["mcts"]["disable_tqdm"]
...
#class NTKScorer(MCTSAgent):
    ...
    #     self.dataset_classes, self.class_permutation = subset_classes(dataset=dataset, samples_per_class=100, device=self.device,
    #                                                         subsample=10)
    ...
    # def _score_network(self, network: nn.Module):
    ...
    #     score = compute_score(network, self.dataset_classes, class_permutation=self.class_permutation, device=self.device)
    #     return score
...
class UCT(MCTSAgent):
    ...
    def __init__(self, root_node: Node, api=None, df=None, params_path=None):
        super().__init__(root_node, api=api, df=df, params_path=params_path)
    ...
    def _get_reward(self, node: Node):
        return node.get_reward(self.api, self.df)
...
class RAVE(UCT):
    ...
    def __init__(self, root_node: Node, api=None, df=None, params_path=None):
        ...
        self.b = None
        super(RAVE, self).__init__(root_node, api=api, df=df, params_path=params_path)
